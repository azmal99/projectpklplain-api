...
class LaporanCSEController extends Controller
{
    ...
    public function index($userId)
    {
        $devices = DB::table('pawoon2.devices')
            ->select(DB::raw('distinct pawoon2.devices.name as device_name, pawoon2.devices.d_id, pawoon2.companies.name as company_name, pawoon2.users.email as email_owner, pawoon2.transactions.device_timestamp as last_transaction, pawoon2.roles_2.name as tier'))
            ...
            ->join('pawoon2.users', 'pawoon2.users.u_id', '=', 'pawoon2.user_has_companies.user_id')
    ...
    public function showoutlets($userId)
    {
        $outlets = DB::table('pawoon2.outlets')
            ->select(DB::raw('distinct pawoon2.outlets.o_id, pawoon2.outlets.uuid, pawoon2.outlets.name as outlet_name, pawoon2.companies.name as company_name, pawoon2.users.email as email_owner, pawoon2.roles_2.name as tier'))
            //DB::raw("SUM(if(DATE(device_timestamp) = '2020-01-13', final_amount,0))as grand_total"))
            ->where('pawoon2.roles.r_id', '1')
            ->where('pawoon1.user_companies.user_id', $userId)
            ->join('pawoon2.companies', 'pawoon2.outlets.company_id', '=', 'pawoon2.companies.c_id')
            ->join('pawoon2.transactions', 'pawoon2.outlets.o_id', '=', 'pawoon2.transactions.outlet_id')
            ->join('pawoon2.user_has_companies', 'pawoon2.companies.c_id', '=', 'pawoon2.user_has_companies.company_id')
            ->join('pawoon2.users', 'pawoon2.users.u_id', '=', 'pawoon2.user_has_companies.user_id')
            ->join('pawoon1.user_companies', 'pawoon1.user_companies.company_id', '=', 'pawoon2.companies.c_id')
    ...
    //OutletsReport
    public function reportoutlet($oId)
    {
        ...
        ->whereDate('device_timestamp', '2020-01-13')*/
            ->where('pawoon2.outlets.o_id', $oId)
            ->join('pawoon2.outlets', 'pawoon2.transactions.outlet_id', '=', 'pawoon2.outlets.o_id')
            ->join('pawoon2.companies', 'pawoon2.outlets.company_id', '=', 'pawoon2.companies.o_id')
            ->join('pawoon2.devices', 'pawoon2.transactions.device_id', '=', 'pawoon2.devices.d_id')
            ->get();
